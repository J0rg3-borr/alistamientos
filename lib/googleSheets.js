...
async function appendRow(values) {
  ...
  const sheetNameRaw = (process.env.SHEET_NAME || 'Alistamientos').trim();
  // Si el nombre contiene espacios o caracteres especiales, envolver en comillas simples
  // Para A1 notation, las comillas simples internas deben duplicarse: "O'Brien" -> 'O''Brien'
    let sheetNameRaw = process.env.SHEET_NAME || 'Alistamientos';
    // Normalizar y eliminar caracteres invisibles (NBSP, zero-width, BOM), luego recortar
    ...
    // Envolver siempre en comillas simples para evitar errores de parseo
